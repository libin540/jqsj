...
const getMods = async () => {
	...
	let tempMyMods = await getFavoriteMenuList();
	
	myModsName.value = tempMyMods.map((v: MenuOutput) => v.name);
	...
	console.log(myMods.value)
};

// 递归拿到所有可显示非iframe的1级菜单
const filterMenu = (map: MenuOutput[]) => {
	map.forEach((item: MenuOutput) => {
		...
		if (item.children && item.children.length > 0) {
			item.cpath = item.children[0].path
